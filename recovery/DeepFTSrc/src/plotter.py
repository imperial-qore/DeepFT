...
class Model_Plotter():
	...
	def plot(self, accuracy_list, epoch):
		...
		self.class_score_list = [i[4] for i in accuracy_list]
		self.plot1('Loss', self.loss_list)
		self.plot1('Factor', self.factor_list)
		self.plot2('Anomaly Loss', 'Class Loss', self.aloss_list, self.tloss_list)
	...
	def plot_tsne(self, name1, data1, labels1):
		...
		target_ids = range(K+1); labs = ['NA'] + [f'k_{i}' for i in range(K)]
		...
		ax.legend(ncol=K+1, loc=9, bbox_to_anchor=(0.5, 1.2))
